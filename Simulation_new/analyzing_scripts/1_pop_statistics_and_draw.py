...
# This is for compling and draw the results generated by sim_one_population.py
...
def getOuterInner(treeStr):
    ...
    for elem in elements:
        if "(" not in elem and ")" not in elem:
            outer_inner["outer"] = elem.strip().split(':')[0].split("_")[-1]
        else:
            outer_inner["inner"].append(re.sub('\)$', '', re.sub('^\(', '', elem.strip().split(':')[0])).strip().split("_")[-1])
    return outer_inner
