...
# Before running this script, run the statistics.py first
...
def draw(divergence_time_2_dir, outFig, axes, colorList, firstline):
    ...
    for i, selection_coefficient in enumerate(sorted(os.listdir(divergence_time_2_dir), key=lambda i:abs(float(i.split("=")[-1])))):
        ...
        for statFile in sorted(glob.glob(os.path.join(subdir, "stat#*.csv")),key=sort_lambada):
        ...
        for k, topo in enumerate(topoList):
            ...
            dfForATop = pd.concat(seriesList, axis=1, keys=[chr(each+97).upper() for each in xrange(len(rawDfList))])
        ...
        t12 = int(float(statFile.strip(".csv").split("_")[-1]))
        n12 = int(float(statFile.strip(".csv").split("_")[-3]))
        f_concordant = 1 - 2*np.exp(-t12/n12)/3
        f_disconcordant = np.exp(-t12/n12)/3
        x_down, x_up = ax.get_xlim()
        ax.plot([x_down, x_up], [f_concordant]*2, color="#75b84f", linestyle='--', linewidth=1)
        ax.plot([x_down, x_up], [f_disconcordant]*2, color="dimgray", linestyle='--', linewidth=1)
    # plt.show()


def drawAll(rootDir, colorList):
    outFig = os.path.join(rootDir, "result.pdf")
